...
export const welcome = (req, res) => {
    return res.status(200).json({ message: "User Page", success: true });
}
...
export const getAllUsers = async (req, res) => {
    try {
        ...
        if (!isNaN(keyword) && keyword.trim() !== "") query.$or.push({ phoneNumber: Number(keyword) });

        const users = await User.find(query, "-password").sort({ createdAt: -1 });
        if (!users.length) return res.status(404).json({ message: "Users not found", success: false });
...
export const getUserByName = async (req, res) => {
    try {
        ...
        const users = await User.find({ fullname: { $regex: nameQuery, $options: "i" } });
        if (!users.length) return res.status(404).json({ message: "No users found", success: false });
        return res.status(200).json({ users, success: true });
    } catch (error) {
        console.error(error);
        return res.status(500).json({ message: "Internal server error", success: false });
...
// ------------------ Update Status & Food Counters ------------------
const updateFoodCounterAndFoods = async (userId, status) => {
    const counters = await food_Counter.find({ userId });
    if (!counters.length) return;

    await food_Counter.updateMany({ userId }, { status: status === "Active" ? "Open" : "Closed" });
    await food.updateMany({ food_counter_id: { $in: counters.map(c => c._id) } }, { status: status === "Active" ? "Available" : "Unavailable" });
};
...
export const updateUserStatus = async (req, res) => {
    try {
        ...
        const user = await User.findById(userId);
        if (!user) return res.status(404).json({ message: "User not found", success: false });
        if (user.role !== "food_provider") return res.status(400).json({ message: "User is not a food provider", success: false });
