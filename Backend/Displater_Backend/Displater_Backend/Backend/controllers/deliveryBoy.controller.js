...
// In-memory OTP store (for small apps)
const otpStore = new Map();
...
export const register = async (req, res) => {
  try {
    ...
    const hashed = await bcrypt.hash(password, 10);
    const deliveryBoy = await DeliveryBoy.create({ fullname, email, phoneNumber, password: hashed });

    res.status(201).json({ message: "Registered successfully", success: true, deliveryBoy });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Internal server error", success: false });
  }
...
export const login = async (req, res) => {
  try {
    const { email, password } = req.body;
    const deliveryBoy = await DeliveryBoy.findOne({ email });
    if (!deliveryBoy) return res.status(400).json({ message: "Invalid credentials", success: false });
    ...
    res.cookie("token", token, { httpOnly: true, maxAge: 24 * 60 * 60 * 1000 }).status(200).json({
      message: `Welcome back ${deliveryBoy.fullname}`,
      deliveryBoy,
      success: true,
    });
...
export const updateProfile = async (req, res) => {
  try {
    ...
    if (!deliveryBoy) return res.status(404).json({ message: "Delivery boy not found", success: false });
    ...
    await deliveryBoy.save();
    res.status(200).json({ message: "Profile updated", success: true, deliveryBoy });
  } catch (err) {
...
export const updatePhoto = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Photo updated", success: true, deliveryBoy });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Photo upload failed", success: false });
  }
};
...
export const updateStatus = async (req, res) => {
  try {
    ...
    deliveryBoy.status = status;
    deliveryBoy.assignStatus = status === "Available" ? "Not Assigned" : "Not Assigned";
    await deliveryBoy.save();

    // Assign pending orders immediately if available
    if (status === "Available") {
      const pendingOrders = await Order.find({ status: "Pending" });
      for (const order of pendingOrders) {
        const freeBoy = await DeliveryBoy.findOne({ status: "Available", assignStatus: "Not Assigned" });
        if (!freeBoy) break;
        order.deliveryBoyId = freeBoy._id;
        order.status = "Assigned";
        await order.save();
    ...
    res.status(200).json({ message: `Delivery boy marked ${status}`, success: true });
  } catch (err) {
    console.error(err);
