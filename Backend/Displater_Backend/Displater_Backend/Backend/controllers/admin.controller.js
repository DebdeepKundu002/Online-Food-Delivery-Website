...
export const createAdmin = async (req, res) => {
  try {
    ...
    const hashed = await hashPassword(password);
    const imageUrl = req.file?.path || ""; // Multer file path or blank
...
export const loginAdmin = async (req, res) => {
  try {
    ...
    const token = jwt.sign({ userId: user._id, role: "admin" }, process.env.SECRET_KEY, { expiresIn: "1d" });
    ...
    res.status(200).json({
      success: true,
      message: "Login successful",
      admin: { id: user._id, email: user.email, name: user.name, phone_number: user.phone_number, image: user.image },
    });
  } catch (err) {
...
// ✅ Update Admin Photo (Cloudinary safe)
export const updateAdminPhoto = async (req, res) => {
  try {
    const adminId = req.params.id;
    ...
    const streamUpload = (buffer) =>
      new Promise((resolve, reject) => {
        const stream = cloudinary.uploader.upload_stream({ folder: "admin_profile" }, (error, result) => {
          if (result) resolve(result);
          else reject(error);
        });
        streamifier.createReadStream(buffer).pipe(stream);
    ...
    const updatedAdmin = await admin.findByIdAndUpdate(adminId, { image: result.secure_url }, { new: true });

    return res.status(200).json({ success: true, message: "Profile photo updated", data: updatedAdmin });
  } catch (error) {
    return res.status(500).json({ success: false, message: "Image upload failed", error: error.message });
  }
};

// ✅ OTP & Password Reset remain same
export const otpStore = new Map();
